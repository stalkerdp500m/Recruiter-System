...
const updateNameTeamForm = useForm({
    'action': 'name',
    'teamName': ''
});
...
function teamNameUpdate (team) {
    updateNameTeamForm.teamName = team.name
    updateNameTeamForm.put(route('control.teams.update', { 'id': team.id }), { preserveScroll: true });
    team.changeName = false
}
    ...
    <MainLayout>
        <div class="md:px-10">
            ...
            <div class="flex gap-2">
                <div class="  w-7/12  justify-center flex items-center bg-white rounded-md ">
                    ...
                    <input @input="e => serched(e.target.value)" type="text"
                        class=" h-10  w-full md:w-4/5 focus:ring-0 border-0 " placeholder="Поиск команды по названию">
                </div>
                <div class="w-4/12 ">
            ...
            <div class="bg-white my-2 p-3 rounded-md shadow-md   " v-for="(team, i) in filteredTeamsList" :key="i">
                <div class="flex justify-start cursor-pointer items-center my-2  ">
                    ...
                    <div v-if="!team.changeName" class="flex cursor-pointer"
                        @click="{team.originName=team.name; team.changeName=true}">
                        <h3 class="text-xl">{{ team.name }} </h3>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="h-3 w-3 mx-2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                        </svg>
                    </div>
                    <div v-else class=" flex items-center flex-wrap">
                        <BreezeInput id="name" type="text" class="my-2  " v-model="team.name" required autofocus />
                        <div @click="teamNameUpdate( team)" class="p-2 bg-green-500 rounded-md mx-4"><svg
                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" class="w-6 h-6 mx-2  ">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg></div>
                        <div @click="team.name=team.originName; team.changeName=false"
                            class="p-2 bg-red-500 rounded-md "><svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mx-2">
                                <path stroke-linecap="round" stroke-linejoin="round"
